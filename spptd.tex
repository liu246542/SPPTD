...
In real practice such as sparse crowdsensing, it is common that the sensory data collected by some participants are sparse, which implies that the privacy of whether or not a participant collects a specific data also need to be protected.
...
To address these issues, we design a privacy-preserving truth discovery scheme by employing additively homomorphic cryptosystem with two non-colluding servers.
% By employing additively homomorphic cryptosystem with two non-colluding servers, we design a privacy-preserving truth discovery scheme for sparse data scenarios
% to guarantee strong privacy in sparse data scenarios.
...
However, the CRH algorithm requires that each worker has to provide sensory data over all sensed objects, which means it fails to consider the situation when a worker only provides sensory data over partial sensed objects.
...
To collect sensory data for all sensed objects, workers need to access into all distinct sub-areas within a specific period, which is impractical.
...
Generally, these schemes can be categorized into single-server setting and two-server setting.
...
The first privacy-preserving truth discovery was presented by Miao et al.~\cite{miao_cloud-enabled_2015} based on single-server setting.
...
They utilized threshold paillier cryptosystem to encrypt the sensory data so that the workers' privacy can be preserved.
...
However, while under the single server setting, workers have to participate the iterations which means that workers need to suffer additional computing and communication overheads.
...
Their proposed scheme can realize strong privacy protection for workers' sensory data by adopting Garbled Circuit but it fails to take the privacy issues appeared in sparse data scenarios into account.
...
Generally, our system model consists of two types of entities: a number of workers and two non-colluding servers. %用图说明
The specific explanations of these entities are listed as follows.
% worker 的作用， cloud server 的作用
\begin{itemize}
  \item \textbf{Workers:} Workers refers to the mobile devices in a mobile crowdsensing system. They are responsible for collecting sensory data of different objects and upload these sensory data to servers. Since most of mobile devices are resource-limited, it is not necessary for each worker to collect sensory data of all objects.
...
In this paper, suppose that there are $M$ objects need to be sensed, and the number of workers is $K$.
...
In other words, the indicator vector for each worker is also need to be preserved.
Therefore, the main goal is to protect workers' privacy including sensory data, indicator vector, and weight for each worker.
Besides, considering that most of workers are resource-limited, it is necessary to reduce the computation and communication overheads on the workers' side.
...
Since the truth discovery algorithm considers estimating ground truth from existing sensory data, the issues of inferring missing data are out of the scope of this paper and we do not consider them in this work.
...
In general, truth discovery algorithm CATD consists of two parts: weight update and truth update.
...
\begin{itemize}
  \item $\mathsf{Setup}(1^\kappa)\to pp$: Taken the input of security parameter $\kappa$, the algorithm returns the public parameter $pp$. Unless otherwise stated, $pp$ is implicitly fed in following algorithms.
...
We called that above public-key cryptosystem is additively homomorphic if it satisfies the following properties in probabilistic polynomial time.
...
Overall, workers only need to split different data into two parts, and then uploads to two servers respectively.
...
Iteration phase consists of secure weight update phase and secure truth update phase.
...
Note that if this phase is executed for the first time, the initial ground truths $\{x_m^*\}_{m=1}^M$ is chosen randomly.
...
But the encrypted CATD cannot guarantee the privacy of indicator privacy, which means server can learn the information of which objects are observed by a worker. 
...
In this section, we evaluate the performance of the propose scheme in terms of accuracy, convergence, computation overhead and communication overhead.
...
As shown in Fig.~\ref{fig:rmse1}, we can observe that the proposed scheme achieves the similar accuracy to the baseline algorithm CATD.
Additionally, the estimate truths is closer to the real ground truths with the increase of number of workers.
\begin{figure}[htbp]
  \centering 
...
TABLE~\ref{tab:computation} lists the computation overhead of workers and $S_0$ in non-iteration phases including report phase and pre-processing phase with the different number of objects and workers.
 % ranging from 20 to 100.
% Here, the number of workers is fixed as 10.
We observe the computation cost of workers is negligible compared to $S_0$ since there is no heavy cryptographic operations on the workers' side.
...
It is clear that the communication overhead in iteration phase is more less than that in non-iteration phase.
% Since 
% Although applying GC is a straightforward method to achieve strong privacy preservation, the generation of GC is still a cumbersome task.
% Considering that 
...
We demonstrate that the both computation and communication overheads on the workers' side are lightweight, and the communication overhead between two servers is also acceptable for truth discovery in reality.
...
After that, we present a scheme to guarantee these requirements by utilizing additively homomorphic cryptosystem based on CATD framework.
...
Extensive experiments also indicate that the computation overhead of workers and the communication overhead in iteration phase are lightweight enough, which implies that the proposed scheme is practical in the real mobile crowdsensing systems.
