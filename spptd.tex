...
In some specific applications such as the sparse mobile crowdsensing, workers can only contribute sensory data on small part of objects, which implies that the information of whether or not a worker collects specific data should also be protected.
...
Finally, through detailed security analysis and extensive experiments, we demonstrate that our proposed scheme can satisfy strong privacy-preserving requirements with low computation and communication overhead.
\end{abstract}
\begin{IEEEkeywords}
...
So far, these schemes can only be applied to the situation where each worker has to provide sensory data for the whole sensing tasks.
...
To the best of our knowledge, there has been little discussion about privacy-preserving truth discovery in such a sparse data situation.
...
After that, more and more schemes~\cite{zhang_lptd_2019,zhang_reliable_2019,xue_inpptd_2020} are designed under the two-server setting.

Besides, no matter the single-server-based schemes or two-server-based schemes, all the previously mentioned methods are based on the {\bf C}onflict {\bf R}esolution on {\bf H}eterogeneous Data (CRH) algorithm~\cite{li_resolving_2014}, which is a representative truth discovery algorithm in recent years.
Due to the limitation that each worker has to provide sensory data of all sensing tasks in CRH algorithm, the CRH-based privacy-preserving truth discovery schemes fail to handle the sparse data.
...
Fortunately, there is another truth discovery algorithm called CATD~\cite{li_confidence-aware_2014} for handling sparse data.
Zheng et al.~\cite{zheng_learning_2018} designed an encrypted truth discovery based on CATD with two non-colluding servers, where all procedures are conducted in the encrypted domain.
...
Taking the sparse mobile crowdsensing~\cite{wang_sparse_2020} as an example, the sensing tasks are distributed in different locations of a spatial area, and workers cannot execute all tasks within a short period which leads to sparse sensory data.  
The research pointed out that the information of which tasks are executed by a worker can disclose the worker's location privacy.
However, their work only focuses on the privacy requirements in sparse mobile crowdsensing while not takes the truth discovery into account.
...
We further present the comparison between our proposed scheme with several state-of-the-art privacy-preserving truth discovery schemes.
In this work, we use an an indicator to mark which tasks are executed by a worker.
As listed in TABLE~\ref{tab:security}, only the encrypted CATD and our proposed scheme can deal with the sparse data scenarios.
...
Finally, we describe the threat model of our system.
...
\begin{itemize}
  \item \textbf{Workers:} Workers refers to the mobile devices in a mobile crowdsensing system. They are responsible for collecting sensory data of different sensing objects and upload these sensory data to servers. Since most mobile devices are resource-limited, it is not necessary for each worker to collect sensory data of all sensing objects.
  \item \textbf{Servers:} Servers are responsible for analyzing the sensory data for sensing objects. More specifically, after receiving the sensory data uploaded by workers, servers start to estimate the truth for each sensing object. In general, we assume that the servers have sufficient computation and storage capabilities. Since our proposed scheme is under two-server setting, we use $S_0$ and $S_1$ to denote these two servers respectively.
\end{itemize}
...
In the proposed scheme, we assume that all entities are semi-honest, which means that both workers and servers will honestly execute the protocols, but they also try to infer private information about participants.
...
Besides, similar to other two-sever-based schemes~\cite{zhang_lptd_2019,zhang_reliable_2019}, the two servers are assumed not to collude with each other in this system.
...
\end{equation*}
Note that $\chi^2$ denotes the Chi-squared distribution, and the system-wide constant $\alpha$ is known as the significance level which is usually a small number such as 0.05~\cite{li_confidence-aware_2014}.
...
In real practice, weight update and truth update should be executed iteratively to achieve more accurate truths until the convergence criteria are met.
...
\begin{itemize}
  ...
  \item $\forall m_3\in\mathcal{M}$, given a constant $a$ and a ciphertext $c_3=\mathsf{Enc}_{pk}(m_3)$, it holds that $\mathsf{Dec}_{sk}(a \otimes c_3) = a \times m_3$.
\end{itemize}
...
In this paper, we only pay attention to the situation that $n=2$.
That is, the data owner can randomly split a secret data $x$ into two additive shares $x_0$ and $x_1$ where $x = x_0 + x_1$.
...
In brief, $\forall m_1, m_2, m_3\in\mathcal{M}$, given the corresponding ciphertexts $c_1,c_2,c_3$ and a constant $a$, it always holds that 
...
Generally speaking, workers only need to generate shares and then upload them to two servers respectively.
...
The whole procedure can be divided into 5 phases: {\em Initialization Phase}, {\em Report Phase}, {\em Pre-Processing Phase}, {\em Secure Weight Update Phase} and {\em Secure Truth Update Phase}.
...
Since workers do not take part in the iteration phase, we only need to consider that the privacy of $x_m^k$, $\phi_m^k$ and $w_k$ are not disclosed to $S_0$ or $S_1$ for each worker $k$.
